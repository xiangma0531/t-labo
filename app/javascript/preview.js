document.addEventListener('DOMContentLoaded', function(){
  // 新規投稿・編集ページのフォームを取得
  const postForm = document.getElementById('source_form');
  ...
  const buildNewFileField = () => {
    ...
    newFileField.setAttribute('name', 'source[images][]');

    // 最後のfile_fieldを取得
    const lastFileField = document.querySelector('input[type="file"][name="source[images][]"]:last-child');
  ...
  // input要素を取得
  const fileField = document.querySelector('input[type="file"][name="source[images][]"]');
